...
class Ball {
  ...
  collideWithPaddle() {
    if (mouseX - paddle.width / 2 < this.x && this.x < mouseX + paddle.width / 2 && this.y > 500 && this.y < 510) {
      this.vy = -this.vy;
    }
  }
  collideWithTargets() {
    for (let i = 0; i < targets.length; i++) {
      ...
      if (this.bottomEdge() > target.topEdge() && this.topEdge() < target.topEdge()) {
        if (this.rightEdge() > target.leftEdge() && this.leftEdge() < target.rightEdge()) {
          this.vy = -this.vy;
          targets.splice(i, 1);
          this.vy *= 1.1;
          this.vx *= 1.15;
