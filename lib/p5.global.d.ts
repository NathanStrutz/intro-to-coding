...
declare global {
     ...
     *   colors are interpolated depends on the current
     *   color mode.
     ...
     *   colors: C = A*factor + B. This is the default
     *   blending mode.
     *   - ADD - sum of A and B
     *   - DARKEST - only the darkest color succeeds: C =
     *   min(A*factor, B).
     *   - LIGHTEST - only the lightest color succeeds: C
     *   = max(A*factor, B).
